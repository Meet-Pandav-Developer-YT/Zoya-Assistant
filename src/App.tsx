...
import { Mic, MicOff, Loader2, Volume2, VolumeX, Keyboard, Send, Trash2, Plus, Minus, Volume1 } from "lucide-react";
...
export default function App() {
  ...
  useEffect(() => {
    if (liveSessionRef.current) {
      liveSessionRef.current.setVolume(volume);
    }
  }, [volume]);
  ...
  return (
    <div className="h-[100dvh] w-screen bg-[#050505] text-white flex flex-col items-center justify-between font-sans relative overflow-hidden m-0 p-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1 bg-white/5 rounded-full px-2 py-1 border border-white/10">
            ...
            <div className="flex items-center gap-1.5 px-1 min-w-[3rem] justify-center">
              {volume === 0 ? <VolumeX size={14} className="text-red-400" /> : 
               volume < 0.5 ? <Volume1 size={14} className="text-cyan-400" /> : 
            ...
            <button
              onClick={() => setVolume(Math.min(2, volume + 0.1))}
              className="p-1.5 rounded-full hover:bg-white/10 transition-colors text-white/60 hover:text-white"
              title="Volume Up"
            >
              <Plus size={14} />
      ...
      <footer className="absolute bottom-0 left-0 w-full flex flex-col items-center justify-center pb-6 md:pb-8 z-20 shrink-0 gap-4">
        ...
        <div className="mt-4 text-[10px] md:text-xs text-white/30 font-mono tracking-widest text-center px-4">
          © 2026-27 | Designed & Maintenance By TechnoSite Developers | All Right Reserved.
        </div>
      </footer>
    </div>
  );
